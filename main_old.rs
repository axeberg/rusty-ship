...
/// This function is called when the websocket handshake is successful
async fn handle_upgrade(socket: WebSocket, from: SocketAddr) {
    ...
    info!("Creating a channel for managing asynchronous tasks ➕");
    let (tx, mut rx): (UnboundedSender<Message>, UnboundedReceiver<Message>) =
        mpsc::unbounded_channel();

    // // Spawn a task that forwards messages from the client to the server
    // // If an error is thrown it closes the connection
    tokio::spawn(forward_messages(receiver, tx, from));

    /* /// Spawn tasks that send and receive messages to clients
    /// If an error is thrown it closes the connection
